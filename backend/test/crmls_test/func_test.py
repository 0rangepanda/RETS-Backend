...
class TestLibretsFunction(unittest.TestCase):
    ...
    # test streetname of a property
    def test_streetname(self):
        print("\nStreetname test:")
        # create request
        streetname = ["StreetNumberNumeric","StreetDirPrefix","StreetDirSuffix",
                      "StreetName","StreetSuffixModifier","StreetSuffix","StreetNumber",]
        request = self.session.CreateSearchRequest(
            "Property",
            "Residential",
            "(StandardStatus=A),(PostalCode=90006)"
        )
        request.SetStandardNames(True)
        request.SetSelect(",".join(streetname))
        request.SetLimit(librets.SearchRequest.LIMIT_DEFAULT)
        request.SetOffset(librets.SearchRequest.OFFSET_NONE)
        request.SetCountType(librets.SearchRequest.RECORD_COUNT_AND_RESULTS)
        ...
        while results.HasNext():
            name = []
            for item in streetname:
                name.append(results.GetString(item))
            fullname = " ".join(name)
            fullname = re.sub(' +', ' ', fullname) # remove dup space
            print("Full Streetname: " + fullname)
